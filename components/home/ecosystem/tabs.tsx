    ...
    <ul className="flex w-full items-center justify-between gap-4">
      {tabsData.map((tab, index) => (
        <li key={tab.title} onClick={() => setSelectedTab(index)}>
          <button
            className={`${selectedTab === index && "bg-black/80 backdrop-blur-lg"} flex flex-col items-center gap-4 rounded-t-lg px-4 py-2 transition-all duration-300 ease-in-out hover:bg-black/80 hover:backdrop-blur-sm lg:px-16 lg:py-7`}
          >
